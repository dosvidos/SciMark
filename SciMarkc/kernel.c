...
#include "array.h"
    ...
    double kernel_measureFFT(int N, double mintime, Random R)
    {
		...
		printf("FFT stopwatch %f flops %f cycles %d\n",Stopwatch_read(Q),FFT_num_flops(N),cycles);
		printf("FFT stopwatch %f flops %f cycles %f\n",Stopwatch_read(Q) * 1.0e-6,FFT_num_flops(N) * cycles, result);
        Stopwatch_delete(Q);
        free(x);
        return result;
    }
